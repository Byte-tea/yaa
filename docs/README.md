...
- 消息：一般包含智能体消息、系统错误消息、用户发送的消息、工具回应消息、大模型的回复消息等。
  - 角色：如 `user`、`assistant`、`system`、`tool`、`error` 等。
  - 内容：消息的内容。
...
- 消息：一般包含智能体消息、系统错误消息、工具回应消息、大模型的回复消息等。
  - 角色：如 `assistant`、`system`、`tool`、`error` 等。
  - 内容：消息的内容。
- 用量：如大模型的用量。
...
流式传输时的智能体回复数据主要包含两个部分：

- 属性：
  - 编号：用于唯一标识会话数据。
  - 状态：`进行中`、`已中断`、`已完成`。
- 消息：一般为智能体消息、系统错误消息、工具回应消息、大模型的回复消息等。
  - 角色：如 `assistant`、`system`、`tool`、`error` 等。
  - 内容：消息的内容。
